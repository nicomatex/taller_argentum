...
Camera::Camera(const RenderizableObject &follow_component, int map_size,
...
SDLArea Camera::_get_render_area(RenderizableObject *component) {
    ...
    int camera_offset_x =
        (follow_component.get_transition_offset_x() * tile_size) /
        OFFSET_GRANULARITY;
    int camera_offset_y =
        (follow_component.get_transition_offset_y() * tile_size) /
        OFFSET_GRANULARITY;
...
void Camera::render_components(std::vector<RenderizableObject *> components) {
    ...
    /* Actualiza el estado visual de todos los componentes luego de
     * renderizar.*/
    for (auto component = components.begin(); component != components.end();
         ++component) {
        (*component)->update();
