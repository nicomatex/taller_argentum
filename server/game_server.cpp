...
GameServer::GameServer() : session(map,event_queue),accepter(Socket("8080", 10),session) {}
...
void GameServer::game_loop(){
    while(running){
        ...
        std::cout << "Jugador en " << map.get_position_map()[1].x << " - " << map.get_position_map()[1].y << std::endl;
