...
std::unordered_map<race_type_t, nlohmann::json> AttributeManager::race_modif_map;
std::unordered_map<race_type_t, nlohmann::json> AttributeManager::race_multi_map;

std::unordered_map<class_type_t, std::string> AttributeManager::class_names_map;
std::unordered_map<class_type_t, nlohmann::json> AttributeManager::class_multi_map;
...
void AttributeManager::init(const char *stats_file,
    ...
    if (!classes_istream.is_open()) throw std::exception();
...
stats_t AttributeManager::create_stats(race_type_t race_type) {
	if (!race_exists(race_type)) throw std::exception();
...
int AttributeManager::get_class_hp_multiplier(class_type_t class_type) {
    if (!class_exists(class_type)) throw std::exception();
